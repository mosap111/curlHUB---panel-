    ...
    try {
        ...
        if (res.success) {
            ...
            if (res.domains.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 20px; font-size:16px;">لم يتم إضافة أي نطاقات بعد. انقر على "إضافة نطاق جديد" للبدء.</td></tr>';
            } else {
                res.domains.forEach(d => {
                    ...
                    if (d.ssl) {
                        ...
                        const expiryColor = (daysLeft !== null && daysLeft < 15) ? 'var(--text-danger)' : (daysLeft !== null && daysLeft < 30) ? 'var(--text-warning)' : 'var(--text-success)';
                        ...
                        sslHtml = `
                            <div style="display:flex; flex-direction:column; gap:2px;">
                                <span class="badge" style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34,197,94,0.3); color: var(--text-success); align-self: flex-start; padding: 2px 8px;">
                        ...
                        sslHtml = `
                            <span class="badge" style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--text-danger); padding: 2px 8px;">
                    ...
                    let httpsHtml = d.force_https ? '<span style="font-size:10px; padding:2px 6px; background: rgba(56,189,248,0.1); color: var(--text-info); border-radius:4px; margin-right:5px; border:1px solid rgba(56,189,248,0.3);">Force HTTPS</span>' : '';
                    let urlLink = d.force_https || d.ssl ? `https://${d.domain}` : `http://${d.domain}`;

                    const tr = document.createElement('tr');
                    tr.style.transition = 'all 0.2s';
                    tr.onmouseenter = () => tr.style.backgroundColor = 'var(--bg-subtle-hover)';
                    tr.onmouseleave = () => tr.style.backgroundColor = 'transparent';
                    
                    tr.innerHTML = `
                        <td>
                            <div style="display:flex; flex-direction:column; gap:4px;">
                                <a href="${urlLink}" target="_blank" style="font-weight: bold; color: var(--text-info); text-decoration: none; display: flex; align-items: center; gap: 5px;">
                                    ${escapeHtml(d.domain)}
                        ...
                        <td style="color: var(--text-secondary); font-family: monospace; font-size: 13px; background: var(--bg-subtle); border-radius: 6px; padding: 6px 10px; max-width: 200px; overflow-x: auto;">
            ...
            tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; color: var(--text-danger); padding: 20px;">خطأ: ${res.error}</td></tr>`;
        }
    } catch (e) {
        console.error(e);
        tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-danger); padding: 20px;">خطأ في الاتصال بالسيرفر</td></tr>';
    }
}
